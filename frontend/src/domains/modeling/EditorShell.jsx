...
export default function EditorShell({ project, userId = "session", onProjectChange, onBack }) {
  ...
  async function previewFile(event) {
    ...
    try {
      ...
      const report = await previewInterchange(project.id, file, interchangeProfiles[0].id, "overlay", selectedDiagram);
      setImportFile(file); setImportReport(report); setStatus("Revisa los objetos que se añadirán al diagrama");
  ...
  async function confirmImport() {
    ...
    try {
      await importProjectFile(project.id, importFile, interchangeProfiles[0].id, "overlay", selectedDiagram);
      setImportFile(null); setImportReport(null); await loadModel(selectedDiagram); setStatus("Objetos importados y añadidos al diagrama");
    } catch (cause) { setStatus(cause?.detail || cause?.errors?.[0] || "No se pudo añadir la importación"); }
  }
  ...
  return <main className="flex h-screen min-h-[640px] flex-col overflow-hidden bg-slate-100 text-slate-800">
    ...
    {importReport && <div className="fixed inset-0 z-40 grid place-items-center bg-slate-900/40 p-4"><div className="w-full max-w-lg rounded-2xl bg-white p-6 shadow-2xl"><h2 className="text-xl font-bold">Añadir al diagrama actual</h2><p className="mt-2 text-sm text-slate-500">Los objetos importados se colocarán sobre el diagrama actual sin reemplazar los que ya existen. Después podrás moverlos y acomodarlos.</p><div className="mt-5 grid grid-cols-3 gap-3 text-center"><div className="rounded-xl bg-slate-50 p-3"><strong className="block text-xl">{importReport.elements?.length || 0}</strong><span className="text-xs">elementos nuevos</span></div><div className="rounded-xl bg-slate-50 p-3"><strong className="block text-xl">{importReport.matches?.length || 0}</strong><span className="text-xs">IDs repetidos</span></div><div className="rounded-xl bg-amber-50 p-3"><strong className="block text-xl">{importReport.warnings?.length || 0}</strong><span className="text-xs">advertencias</span></div></div>{importReport.warnings?.length > 0 && <ul className="mt-4 max-h-32 list-disc overflow-auto pl-5 text-xs text-amber-700">{importReport.warnings.slice(0, 8).map((warning, index) => <li key={index}>{typeof warning === "string" ? warning : warning.message || warning.code}</li>)}</ul>}<div className="mt-6 flex justify-end gap-2"><button className="rounded-lg px-4 py-2" type="button" onClick={() => { setImportFile(null); setImportReport(null); }}>Cancelar</button><button className="rounded-lg bg-teal-700 px-4 py-2 font-semibold text-white" type="button" onClick={confirmImport}>Añadir al diagrama</button></div></div></div>}
    ...
    {aiOpen && <AiAssistantPanel
      ...
      selectedElementIds={selectedNode?.data?.elementId ? [String(selectedNode.data.elementId)] : []}
      canApply={canEdit}
      onClose={() => setAiOpen(false)}
      onApplied={async () => {
        await loadModel(selectedDiagram);
        setStatus("Cambios de IA aplicados y sincronizados");
      }}
    />}
  </main>;
}
