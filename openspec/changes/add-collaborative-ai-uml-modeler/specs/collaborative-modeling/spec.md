...
### Requirement: Invitación comprensible y presencia sostenida
El sistema SHALL mostrar el código secreto completo únicamente después de generarlo, SHALL explicar que no podrá recuperarse y SHALL mantener la presencia mediante mensajes periódicos mientras el canal permanezca abierto.

#### Scenario: Copiar un código nuevo
- **WHEN** el propietario genera una invitación
- **THEN** puede copiar el código completo y distingue su vencimiento, usos y estado de los prefijos históricos no reutilizables

#### Scenario: Usuario conectado sin actividad
- **WHEN** un colaborador mantiene el proyecto abierto sin seleccionar elementos durante más de treinta segundos
- **THEN** los demás participantes siguen viéndolo conectado gracias al heartbeat
